...
typedef struct AT1SUCtx {
    ...
    DECLARE_ALIGNED(32, float,
                    fst_qmf_delay)[46];         ///< delay line for the 1st stacked QMF filter
    DECLARE_ALIGNED(32, float,
                    snd_qmf_delay)[46];         ///< delay line for the 2nd stacked QMF filter
    DECLARE_ALIGNED(32, float, last_qmf_delay)[
...
static int at1_unpack_dequant(GetBitContext *gb, AT1SUCtx *su,
    ...
    for (band_num = 0; band_num < AT1_QMF_BANDS; band_num++) {
        for (bfu_num = bfu_bands_t[band_num]; bfu_num < bfu_bands_t[band_num + 1]; bfu_num++) {
            ...
            pos = su->log2_block_count[band_num] ? bfu_start_short[bfu_num]
