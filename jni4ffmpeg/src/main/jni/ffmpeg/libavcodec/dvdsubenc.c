...
static int encode_dvd_subtitles(AVCodecContext *avctx,
    ...
    *q++ = 0xff; // terminating command
    ...
    av_log(NULL, AV_LOG_DEBUG, "subtitle_packet size=%"
    PTRDIFF_SPECIFIER
    "\n", q - outbuf);
    ret = q - outbuf;
...
static const AVOption options[] = {
        {"even_rows_fix", "Make number of rows even (workaround for some players)", OFFSET(even_rows_fix), AV_OPT_TYPE_INT, {.i64 = 0}, 0, 1,
                SE},
        {NULL},
};
