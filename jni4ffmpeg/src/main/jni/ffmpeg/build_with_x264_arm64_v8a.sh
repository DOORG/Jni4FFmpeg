...
export PREFIX=../libbuild/armeabi_v8a #编译结果的目录 最终生成的编译结果

EXTRA_DIR=libx264/armeabi_v8a
# 加入x264编译库
...
${TOOLCHAIN}/bin/aarch64-linux-android-ld -rpath-link=${PLATFORM}/usr/lib -L${PLATFORM}/usr/lib \
...
-nostdlib -Bsymbolic --whole-archive --no-undefined -o ${PREFIX}/libffmpeg.so \
    ...
    -lc -lm -lz -ldl -llog --dynamic-linker=/system/bin/linker ${TOOLCHAIN}/lib/gcc/aarch64-linux-android/4.9.x/libgcc.a
