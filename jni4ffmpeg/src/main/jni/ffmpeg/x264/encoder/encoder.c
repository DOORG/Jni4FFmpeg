    ...
    if (!fseek(f, (int64_t) h->fdec->i_frame * frame_size, SEEK_SET)) {
        for (int p = 0; p < (CHROMA444 ? 3 : 1); p++)
            for (int y = 0; y < h->param.i_height; y++)
                fwrite(&h->fdec->plane[p][y * h->fdec->i_stride[p]], sizeof(pixel),
                       h->param.i_width, f);
        if (!CHROMA444) {
            ...
            if (planeu) {
                ...
                h->mc.plane_copy_deinterleave(planeu, cw, planev, cw, h->fdec->plane[1],
                                              h->fdec->i_stride[1], cw, ch);
...
static void x264_slice_header_init(x264_t *h, x264_slice_header_t *sh,
    ...
    if (!h->mb.b_direct_auto_read && sh->i_type == SLICE_TYPE_B) {
        if (h->fref[1][0]->i_poc_l0ref0 == h->fref[0][0]->i_poc) {
            if (h->mb.b_direct_auto_write)
                sh->b_direct_spatial_mv_pred = (h->stat.i_direct_score[1] >
                                                h->stat.i_direct_score[0]);
            else
    ...
    for (int list = 0; list < 2; list++) {
        if (sh->b_ref_pic_list_reordering[list]) {
            ...
            for (int i = 0; i < h->i_ref[list]; i++) {
                ...
                sh->ref_pic_list_order[list][i].arg =
                        (abs(diff) - 1) & ((1 << sps->i_log2_max_frame_num) - 1);
    ...
    int deblock_thresh = i_qp + 2 * X264_MIN(param->i_deblocking_filter_alphac0,
...
static void x264_slice_header_write(bs_t *s, x264_slice_header_t *sh, int i_nal_ref_idc) {
    ...
    bs_write(s, sh->sps->i_log2_max_frame_num,
             sh->i_frame_num & ((1 << sh->sps->i_log2_max_frame_num) - 1));
    ...
    if (sh->sps->i_poc_type == 0) {
        bs_write(s, sh->sps->i_log2_max_poc_lsb,
                 sh->i_poc & ((1 << sh->sps->i_log2_max_poc_lsb) - 1));
    ...
    if (sh->pps->b_weighted_pred && sh->i_type == SLICE_TYPE_P) {
        sh->b_weighted_pred =
                sh->weight[0][0].weightfn || sh->weight[0][1].weightfn || sh->weight[0][2].weightfn;
        /* pred_weight_table() */
...
static int x264_bitstream_check_buffer_internal(x264_t *h, int size, int b_cabac, int i_nal) {
    if ((b_cabac && (h->cabac.p_end - h->cabac.p < size)) ||
        ...
        memcpy(buf + aligned_size, h->out.p_bitstream + aligned_size,
               h->out.i_bitstream - aligned_size);
...
static int x264_validate_parameters(x264_t *h, int b_open) {
    ...
    if (h->param.i_avcintra_class) {
        ...
        h->param.i_slice_max_mbs =
                ((h->param.i_width + 15) / 16) * ((h->param.i_height + 15) / 16) / 10;
    ...
    int max_slices =
            (h->param.i_height + ((16 << PARAM_INTERLACED) - 1)) / (16 << PARAM_INTERLACED);
    if (h->param.b_sliced_threads)
    ...
    if (h->param.b_bluray_compat) {
        ...
        h->param.analyse.i_weighted_pred = X264_MIN(h->param.analyse.i_weighted_pred,
                                                    X264_WEIGHTP_SIMPLE);
    ...
    h->param.analyse.i_direct_mv_pred = x264_clip3(h->param.analyse.i_direct_mv_pred,
                                                   X264_DIRECT_PRED_NONE, X264_DIRECT_PRED_AUTO);
    if (!h->param.analyse.i_subpel_refine &&
        h->param.analyse.i_direct_mv_pred > X264_DIRECT_PRED_SPATIAL) {
        x264_log(h, X264_LOG_WARNING, "subme=0 + direct=temporal is not supported\n");
        h->param.analyse.i_direct_mv_pred = X264_DIRECT_PRED_SPATIAL;
    }
    h->param.i_bframe = x264_clip3(h->param.i_bframe, 0,
                                   X264_MIN(X264_BFRAME_MAX, h->param.i_keyint_max - 1));
    h->param.i_bframe_bias = x264_clip3(h->param.i_bframe_bias, -90, 100);
    if (h->param.i_bframe <= 1)
        h->param.i_bframe_pyramid = X264_B_PYRAMID_NONE;
    h->param.i_bframe_pyramid = x264_clip3(h->param.i_bframe_pyramid, X264_B_PYRAMID_NONE,
                                           X264_B_PYRAMID_NORMAL);
    h->param.i_bframe_adaptive = x264_clip3(h->param.i_bframe_adaptive, X264_B_ADAPT_NONE,
    ...
    h->param.rc.i_lookahead = x264_clip3(h->param.rc.i_lookahead, 0, X264_LOOKAHEAD_MAX);
    {
        ...
        h->param.rc.i_lookahead = X264_MIN(h->param.rc.i_lookahead,
                                           X264_MAX(h->param.i_keyint_max, bufsize * fps));
    }

    if (!h->param.i_timebase_num || !h->param.i_timebase_den ||
        !(h->param.b_vfr_input || h->param.b_pulldown)) {
    ...
    h->param.analyse.b_mixed_references =
            h->param.analyse.b_mixed_references && h->param.i_frame_reference > 1;
    ...
    if (h->param.analyse.i_subpel_refine >= 10 &&
        (h->param.analyse.i_trellis != 2 || !h->param.rc.i_aq_mode))
    ...
    if (b_open) {
        ...
        if (h->param.analyse.i_mv_range <= 0)
            ...
            h->param.analyse.i_mv_range = x264_clip3(h->param.analyse.i_mv_range, 32,
                                                     512 >> PARAM_INTERLACED);
    }

    h->param.analyse.i_weighted_pred = x264_clip3(h->param.analyse.i_weighted_pred,
                                                  X264_WEIGHTP_NONE, X264_WEIGHTP_SMART);
    ...
    if (h->param.i_lookahead_threads == X264_THREADS_AUTO) {
        if (h->param.b_sliced_threads)
            ...
            int subme = X264_MIN(h->param.analyse.i_subpel_refine / 3, 3) +
            ...
            h->param.i_lookahead_threads =
                    h->param.i_threads / lookahead_thread_div[badapt][subme][bframes];
            ...
            h->param.i_lookahead_threads = X264_MIN(h->param.i_lookahead_threads,
                                                    h->param.i_height / 128);
        }
    }
    h->param.i_lookahead_threads = x264_clip3(h->param.i_lookahead_threads, 1,
                                              X264_MIN(max_sliced_threads,
...
static void mbcmp_init(x264_t *h) {
    ...
    memcpy(h->pixf.mbcmp_unaligned, satd ? h->pixf.satd : h->pixf.sad,
           sizeof(h->pixf.mbcmp_unaligned));
...
static void chroma_dsp_init(x264_t *h) {
    memcpy(h->luma2chroma_pixel, x264_luma2chroma_pixel[CHROMA_FORMAT],
           sizeof(h->luma2chroma_pixel));
...
x264_t *x264_encoder_open(x264_param_t *param) {
    ...
    if (h->param.psz_cqm_file) if (x264_cqm_parse_file(h, h->param.psz_cqm_file) < 0)
        goto fail;
    ...
    CHECKED_MALLOCZERO(h->frames.unused[1],
                       (h->i_thread_frames + X264_REF_MAX + 4) * sizeof(x264_frame_t * ));
    CHECKED_MALLOCZERO(h->frames.current, (h->param.i_sync_lookahead + h->param.i_bframe
                                           + h->i_thread_frames + 3) * sizeof(x264_frame_t * ));
    if (h->param.analyse.i_weighted_pred > 0)
        CHECKED_MALLOCZERO(h->frames.blank_unused,
                           h->i_thread_frames * 4 * sizeof(x264_frame_t * ));
    ...
    memcpy(&h->zigzagf, PARAM_INTERLACED ? &h->zigzagf_interlaced : &h->zigzagf_progressive,
           sizeof(h->zigzagf));
    ...
    h->out.i_bitstream = X264_MAX(1000000, h->param.i_width * h->param.i_height * 4
                                           * (h->param.rc.i_rc_method == X264_RC_ABR ? pow(0.95,
                                                                                           h->param.rc.i_qp_min)
                                                                                     :
                                              pow(0.95, h->param.rc.i_qp_constant) *
                                              X264_MAX(1, h->param.rc.f_ip_factor)));

    h->nal_buffer_size = h->out.i_bitstream * 3 / 2 + 4 +
                         64; /* +4 for startcode, +64 for nal_escape assembly padding */
    CHECKED_MALLOC(h->nal_buffer, h->nal_buffer_size);

    CHECKED_MALLOC(h->reconfig_h, sizeof(x264_t));

    if (h->param.i_threads > 1 &&
        x264_threadpool_init(&h->threadpool, h->param.i_threads, (void *) x264_encoder_thread_init,
                             h))
        goto fail;
    ...
    *h->reconfig_h = *h;
    ...
    const char *profile = h->sps->i_profile_idc == PROFILE_BASELINE ? "Constrained Baseline" :
                          ...
                          h->sps->i_profile_idc == PROFILE_HIGH10 ? (h->sps->b_constraint_set3 == 1
                                                                     ? "High 10 Intra" : "High 10")
                                                                  :
                          h->sps->i_profile_idc == PROFILE_HIGH422 ? (h->sps->b_constraint_set3 == 1
                                                                      ? "High 4:2:2 Intra"
                                                                      : "High 4:2:2") :
                          h->sps->b_constraint_set3 == 1 ? "High 4:4:4 Intra"
                                                         : "High 4:4:4 Predictive";
    char level[4];
    snprintf(level, sizeof(level), "%d.%d", h->sps->i_level_idc / 10, h->sps->i_level_idc % 10);
    if (h->sps->i_level_idc == 9 || (h->sps->i_level_idc == 11 && h->sps->b_constraint_set3 &&
                                     (h->sps->i_profile_idc == PROFILE_BASELINE ||
...
static int x264_encoder_encapsulate_nals(x264_t *h, int start) {
    ...
    for (int i = start; i < h->out.i_nal; i++) {
        h->out.nal[i].b_long_startcode =
                !i || h->out.nal[i].i_type == NAL_SPS || h->out.nal[i].i_type == NAL_PPS ||
                h->param.i_avcintra_class;
...
static inline void x264_reference_check_reorder(x264_t *h) {
    ...
    for (int list = 0; list <= (h->sh.i_type == SLICE_TYPE_B); list++)
        for (int i = 0; i < h->i_ref[list] - 1; i++) {
            ...
            if (h->sh.i_type == SLICE_TYPE_P ? framenum_diff > 0 : list == 1 ? poc_diff < 0 :
                                                                   poc_diff > 0) {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < h->i_ref[0]; j++) {
            if (h->fenc->weight[j][i].weightfn) {
                ...
                if (h->sh.weight[j][i].i_scale == 1 << h->sh.weight[j][i].i_denom &&
                    h->sh.weight[j][i].i_offset == 0)
                    ...
                    if (!i) {
                        h->fenc->weighted[j] =
                                h->mb.p_weight_buf[buffer_next++] + h->fenc->i_stride[0] * i_padv +
                                PADH;
                        //scale full resolution frame
                        if (h->param.i_threads == 1) {
                            pixel *src = h->fref[0][j]->filtered[0][0] -
                                         h->fref[0][j]->i_stride[0] * i_padv - PADH;
                            pixel *dst =
                                    h->fenc->weighted[j] - h->fenc->i_stride[0] * i_padv - PADH;
                            int stride = h->fenc->i_stride[0];
                            int width = h->fenc->i_width[0] + PADH * 2;
                            int height = h->fenc->i_lines[0] + i_padv * 2;
                            x264_weight_scale_plane(h, dst, stride, src, stride, width, height,
                                                    &h->sh.weight[j][0]);
...
static inline void x264_reference_build_list(x264_t *h, int i_poc) {
    ...
    for (int list = 0; list < 2; list++) {
        ...
        do {
            ...
            for (int i = 0; i < h->i_ref[list] - 1; i++) {
                ...
                if (x264_reference_distance(h, h->fref[list][i]) >
    ...
    h->i_ref[0] = X264_MIN(h->i_ref[0],
                           h->param.i_frame_reference); // if reconfig() has lowered the limit
...
static void x264_fdec_filter_row(x264_t *h, int mb_y, int pass) {
    ...
    if (PARAM_INTERLACED && (!h->param.b_sliced_threads || pass == 1))
        for (int p = 0; p < h->fdec->i_plane; p++)
            for (int i = minpix_y >> (CHROMA_V_SHIFT && p);
                 i < maxpix_y >> (CHROMA_V_SHIFT && p); i++)
    ...
    if (h->i_thread_frames > 1 && h->fdec->b_kept_as_ref)
        x264_frame_cond_broadcast(h->fdec, mb_y * 16 +
                                           (b_end ? 10000 : -(X264_THREAD_HEIGHT << SLICE_MBAFF)));
        ...
        if (h->param.analyse.b_psnr) {
            for (int p = 0; p < (CHROMA444 ? 3 : 1); p++)
                h->stat.frame.i_ssd[p] += x264_pixel_ssd_wxh(&h->pixf,
                                                             h->fdec->plane[p] +
                                                             minpix_y * h->fdec->i_stride[p],
                                                             h->fdec->i_stride[p],
                                                             h->fenc->plane[p] +
                                                             minpix_y * h->fenc->i_stride[p],
                                                             h->fenc->i_stride[p],
            ...
            if (!CHROMA444) {
                ...
                x264_pixel_ssd_nv12(&h->pixf,
                                    h->fdec->plane[1] +
                                    (minpix_y >> v_shift) * h->fdec->i_stride[1],
                                    h->fdec->i_stride[1],
                                    h->fenc->plane[1] +
                                    (minpix_y >> v_shift) * h->fenc->i_stride[1],
                                    h->fenc->i_stride[1],
                                    h->param.i_width >> 1, (maxpix_y - minpix_y) >> v_shift, &ssd_u,
                                    &ssd_v);
        ...
        if (h->param.analyse.b_ssim) {
            ...
            h->stat.frame.f_ssim +=
                    x264_pixel_ssim_wxh(&h->pixf,
                                        h->fdec->plane[0] + 2 + minpix_y * h->fdec->i_stride[0],
                                        h->fdec->i_stride[0],
                                        h->fenc->plane[0] + 2 + minpix_y * h->fenc->i_stride[0],
                                        h->fenc->i_stride[0],
                                        h->param.i_width - 2, maxpix_y - minpix_y,
...
static inline void x264_slice_init(x264_t *h, int i_nal_type, int i_global_qp) {
    ...
    if (i_nal_type == NAL_SLICE_IDR) {
        x264_slice_header_init(h, &h->sh, h->sps, h->pps, h->i_idr_pic_id, h->i_frame_num,
                               i_global_qp);
        ...
        if (h->sh.i_num_ref_idx_l0_active != h->pps->i_num_ref_idx_l0_default_active ||
            (h->sh.i_type == SLICE_TYPE_B &&
             h->sh.i_num_ref_idx_l1_active != h->pps->i_num_ref_idx_l1_default_active)) {
            h->sh.b_num_ref_idx_override = 1;
        }
    }

    if (h->fenc->i_type == X264_TYPE_BREF && h->param.b_bluray_compat &&
...
static ALWAYS_INLINE void x264_bitstream_backup(x264_t *h, x264_bs_bak_t *bak, int i_skip,
                                                int full) {
    if (full) {
...
static ALWAYS_INLINE void x264_bitstream_restore(x264_t *h, x264_bs_bak_t *bak, int *skip,
                                                 int full) {
    if (full) {
...
static intptr_t x264_slice_write(x264_t *h) {
     ...
     * other inaccuracies. */
    int overhead_guess =
            (NALU_OVERHEAD - (h->param.b_annexb && h->out.i_nal)) + 1 + h->param.b_cabac + 5;
    int slice_max_size =
            h->param.i_slice_max_size > 0 ? (h->param.i_slice_max_size - overhead_guess) * 8 : 0;
...
#define BS_BAK_ROW_VBV        3
    ...
    if (h->param.b_cabac) {
        ...
        /* init cabac */
        x264_cabac_context_init(h, &h->cabac, h->sh.i_type,
                                x264_clip3(h->sh.i_qp - QP_BD_OFFSET, 0, 51),
        ...
        if (PARAM_INTERLACED) {
            if (h->mb.b_adaptive_mbaff) {
                if (!(i_mb_y & 1)) {
                    ...
                    memcpy(&h->zigzagf,
                           MB_INTERLACED ? &h->zigzagf_interlaced : &h->zigzagf_progressive,
                           sizeof(h->zigzagf));
            ...
            if (total_bits - starting_bits > slice_max_size && !h->mb.b_reencode_mb) {
                if (!x264_frame_new_slice(h, h->fdec)) {
                    ...
                    if (mb_xy <= thread_last_mb &&
                        (thread_last_mb + 1 - mb_xy) < h->param.i_slice_min_mbs) {
                    ...
                    if (mb_xy - SLICE_MBAFF * h->mb.i_mb_stride != h->sh.i_first_mb) {
                        ...
                        if (SLICE_MBAFF) {
                            ...
                            if (i_mb_x)
                                ...
                                h->sh.i_last_mb = (i_mb_y - 2 + !(i_mb_y & 1)) * h->mb.i_mb_stride +
                                                  h->mb.i_mb_width - 1;
                        }
                        else
    ...
    while (h->sh.i_first_mb + SLICE_MBAFF * h->mb.i_mb_stride <= last_thread_mb) {
        ...
        if (!i_slice_num || !x264_frame_new_slice(h, h->fdec)) {
            if (h->param.i_slice_max_mbs) {
                ...
                i_slice_num++;
                h->sh.i_last_mb = (height * i_slice_num + h->param.i_slice_count / 2) /
                                  h->param.i_slice_count * width - 1;
            }
        }
...
static int x264_threaded_slices_write(x264_t *h) {
    ...
    for (int i = 0; i < h->param.i_threads; i++) {
        ...
        int height = h->mb.i_mb_height >> PARAM_INTERLACED;
        t->i_threadslice_start = ((height * i + h->param.i_slice_count / 2) / h->param.i_threads) <<
                                 PARAM_INTERLACED;
        t->i_threadslice_end =
                ((height * (i + 1) + h->param.i_slice_count / 2) / h->param.i_threads) <<
                PARAM_INTERLACED;
    ...
    for (int i = 1; i < h->param.i_threads; i++) {
        ...
        for (int j = 0; j < (offsetof(x264_t, stat.frame.i_ssd) -
                             offsetof(x264_t, stat.frame.i_mv_bits)) / sizeof(int); j++)
...
int x264_encoder_encode(x264_t *h,
    ...
    *pp_nal = NULL;
    ...
    if (h->param.b_sliced_threads) if (x264_threadpool_wait_all(h) < 0)
        return -1;
    ...
    if (h->fenc->i_type == X264_TYPE_IDR) {
        ...
        i_nal_type = NAL_SLICE;
        i_nal_ref_idc = h->param.i_bframe_pyramid == X264_B_PYRAMID_STRICT ? NAL_PRIORITY_LOW
                                                                           : NAL_PRIORITY_HIGH;
        h->sh.i_type = SLICE_TYPE_B;
        x264_reference_hierarchy_reset(h);
    }
    ...
    if (h->frames.i_bframe_delay) {
        ...
        h->fdec->i_dts = h->i_frame > h->frames.i_bframe_delay
                         ? prev_reordered_pts[(h->i_frame - h->frames.i_bframe_delay) %
                                              h->frames.i_bframe_delay]
    ...
    if (h->param.b_sliced_threads) {
        for (int i = 0; i < h->param.i_threads; i++) {
            bs_init(&h->thread[i]->out.bs, h->thread[i]->out.p_bitstream,
                    h->thread[i]->out.i_bitstream);
    ...
    if (h->fenc->b_keyframe) {
        ...
        if (h->param.b_repeat_headers) {
            ...
            if (h->param.i_avcintra_class)
                h->out.nal[h->out.i_nal - 1].i_padding =
                        256 - bs_pos(&h->out.bs) / 8 - 2 * NALU_OVERHEAD;
            ...
            if (h->param.i_avcintra_class)
                h->out.nal[h->out.i_nal - 1].i_padding =
                        256 - h->out.nal[h->out.i_nal - 1].i_payload - NALU_OVERHEAD;
            overhead += h->out.nal[h->out.i_nal - 1].i_payload +
                        h->out.nal[h->out.i_nal - 1].i_padding + NALU_OVERHEAD;
        }
    ...
    for (int i = 0; i < h->fenc->extra_sei.num_payloads; i++) {
        x264_nal_start(h, NAL_SEI, NAL_PRIORITY_DISPOSABLE);
        x264_sei_write(&h->out.bs, h->fenc->extra_sei.payloads[i].payload,
                       h->fenc->extra_sei.payloads[i].payload_size,
    ...
    if (h->fenc->b_keyframe) {
        ...
        if (h->fenc->i_type != X264_TYPE_IDR) {
            int time_to_recovery = h->param.b_open_gop ? 0 :
                                   X264_MIN(h->mb.i_mb_width - 1, h->param.i_keyint_max) +
                                   h->param.i_bframe - 1;
            x264_nal_start(h, NAL_SEI, NAL_PRIORITY_DISPOSABLE);
    ...
    if (h->param.i_avcintra_class) {
        ...
        h->out.nal[h->out.i_nal - 1].i_padding =
                total_len - h->out.nal[h->out.i_nal - 1].i_payload - SEI_OVERHEAD;
        overhead +=
                h->out.nal[h->out.i_nal - 1].i_payload + h->out.nal[h->out.i_nal - 1].i_padding +
                SEI_OVERHEAD;
    }
    ...
    else if ((intptr_t) x264_slices_write(h))
        return -1;
...
static int x264_encoder_frame_end(x264_t *h, x264_t *thread_current,
    ...
    if (h->i_thread_frames > 1 && h->fenc->b_keyframe && h->sps->vui.b_nal_hrd_parameters_present) {
        ...
        memmove(&h->out.nal[idx + 1], &h->out.nal[idx],
                (h->out.i_nal - idx - 1) * sizeof(x264_nal_t));
    ...
    if (h->param.i_avcintra_class) {
        if (x264_check_encapsulated_buffer(h, h->thread[0], h->out.i_nal, frame_size,
                                           frame_size + filler) < 0)
            return -1;
...
void x264_encoder_close(x264_t *h) {
        ...
        if (h->stat.i_frame_count[i_slice] > 0) {
            ...
            if (h->param.analyse.b_psnr) {
                x264_log(h, X264_LOG_INFO,
                         ...
                         h->stat.f_psnr_mean_y[i_slice] / dur, h->stat.f_psnr_mean_u[i_slice] / dur,
    ...
    if (h->stat.i_frame_count[SLICE_TYPE_I] + h->stat.i_frame_count[SLICE_TYPE_P] +
        h->stat.i_frame_count[SLICE_TYPE_B] > 0) {
...
#define SUM3b(p, o) (p[SLICE_TYPE_I][o] + p[SLICE_TYPE_P][o] + p[SLICE_TYPE_B][o])
        ...
        if (PARAM_INTERLACED) {
            ...
            fieldstats[0] = 0;
            if (i_inter)
                fieldstats += sprintf(fieldstats, " inter:%.1f%%",
                                      h->stat.i_mb_field[1] * 100.0 / i_inter);
            if (i_skip)
                fieldstats += sprintf(fieldstats, " skip:%.1f%%",
                                      h->stat.i_mb_field[2] * 100.0 / i_skip);
        ...
        if (h->param.analyse.i_weighted_pred >= X264_WEIGHTP_SIMPLE &&
            h->stat.i_frame_count[SLICE_TYPE_P] > 0)
        ...
        for (int i_list = 0; i_list < 2; i_list++)
            for (int i_slice = 0; i_slice < 2; i_slice++) {
                ...
                for (int i = 0; i <= i_max; i++)
                    p += sprintf(p, " %4.1f%%",
                                 100. * h->stat.i_mb_count_ref[i_slice][i_list][i] / i_den);
