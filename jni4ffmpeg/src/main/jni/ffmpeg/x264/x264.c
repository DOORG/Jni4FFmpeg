...
static const char *const pulldown_names[] = {"none", "22", "32", "64", "double", "triple", "euro",
                                             0};
...
static const cli_pulldown_t pulldown_values[] =
        {
                ...
                [X264_PULLDOWN_EURO]   = {24, {TBT, BT, BT, BT, BT, BT, BT, BT, BT, BT, BT, BT,
                                               BTB, TB, TB, TB, TB, TB, TB, TB, TB, TB, TB, TB},
                                          25.0 / 24.0}
        };
...
#define H2 if( longhelp == 2 ) printf
    ...
    H2("                                  - ultrafast:\n"
        ...
        H0("                                  - ultrafast,superfast,veryfast,faster,fast\n"
    ...
    H2("                                  - film (psy tuning):\n"
        ...
        H0("                                  - psy tunings: film,animation,grain,\n"
    ...
    H2("      --range <string>        Specify color range [\"%s\"]\n"
               "                                  - %s\n", range_names[0],
       stringify_names(buf, range_names));
    ...
    H1("      --muxer <string>        Specify output container format [\"%s\"]\n"
               "                                  - %s\n", muxer_names[0],
       stringify_names(buf, muxer_names));
    H1("      --demuxer <string>      Specify input container format [\"%s\"]\n"
               "                                  - %s\n", demuxer_names[0],
       stringify_names(buf, demuxer_names));
...
static int init_vid_filters(char *sequence, hnd_t *handle, video_info_t *info, x264_param_t *param,
    ...
    if (x264_init_vid_filter("source", handle, &filter, info, param,
                             NULL)) /* wrap demuxer into a filter */
        return -1;
    if (x264_init_vid_filter("resize", handle, &filter, info, param,
...
static int parse(int argc, char **argv, x264_param_t *param, cli_opt_t *opt) {
    ...
    for (optind = 0; ;) {
        ...
        switch (c) {
            ...
            case OPT_MUXER:
                ...
                break;
            case OPT_DEMUXER:
                FAIL_IF_ERROR(parse_enum_name(optarg, demuxer_names, &demuxer),
                              "Unknown demuxer `%s'\n", optarg);
                break;
            case OPT_INDEX:
            ...
            case OPT_QPFILE:
                ...
                if (!x264_is_regular_file(opt->qpfile)) {
                    x264_cli_log("x264", X264_LOG_ERROR,
            ...
            case OPT_TIMEBASE:
                ...
                break;
            case OPT_PULLDOWN:
                FAIL_IF_ERROR(parse_enum_value(optarg, pulldown_names, &opt->i_pulldown),
                              "Unknown pulldown `%s'\n", optarg);
                break;
            ...
            case OPT_DTS_COMPRESSION:
                ...
                break;
            case OPT_OUTPUT_CSP:
                FAIL_IF_ERROR(parse_enum_value(optarg, output_csp_names, &output_csp),
                              "Unknown output csp `%s'\n", optarg);
                // correct the parsed value to the libx264 csp value
#if X264_CHROMA_FORMAT
                static const uint8_t output_csp_fix[] = { X264_CHROMA_FORMAT, X264_CSP_RGB };
#else
                static const uint8_t output_csp_fix[] = {X264_CSP_I420, X264_CSP_I422,
                                                         X264_CSP_I444, X264_CSP_RGB};
#endif
                param->i_csp = output_csp = output_csp_fix[output_csp];
                break;
            case OPT_INPUT_RANGE:
                FAIL_IF_ERROR(parse_enum_value(optarg, range_names, &input_opt.input_range),
                              "Unknown input range `%s'\n", optarg);
                input_opt.input_range += RANGE_AUTO;
                break;
            case OPT_RANGE:
                FAIL_IF_ERROR(parse_enum_value(optarg, range_names, &param->vui.b_fullrange),
                              "Unknown range `%s'\n", optarg);
        ...
        if (b_error) {
            const char *name =
                    long_options_index > 0 ? long_options[long_options_index].name : argv[optind -
                                                                                          2];
    ...
    /* Get the file name */
    FAIL_IF_ERROR(optind > argc - 1 || !output_filename,
                  "No %s file. Run x264 --help for a list of options.\n",
                  optind > argc - 1 ? "input" : "output");

    if (select_output(muxer, output_filename, param))
        return -1;
    FAIL_IF_ERROR(cli_output.open_file(output_filename, &opt->hout, &output_opt),
    ...
    if (tcfile_name) {
        ...
        FAIL_IF_ERROR(timecode_input.open_file(tcfile_name, &opt->hin, &info, &input_opt),
                      "timecode input failed\n");
        cli_input = timecode_input;
    }
    else
        FAIL_IF_ERROR(!info.vfr && input_opt.timebase,
    ...
    if (!tcfile_name && input_opt.timebase) {
        ...
        int ret = sscanf(input_opt.timebase, "%"SCNu64"/%"SCNu64, &i_user_timebase_num,
                         &i_user_timebase_den);
...
#else
        x264_cli_log("x264", X264_LOG_WARNING,
...
static int64_t print_status(int64_t i_start, int64_t i_previous, int i_frame, int i_frame_total,
                            int64_t i_file, x264_param_t *param, int64_t last_ts) {
    char buf[200];
    ...
    double bitrate;
    if (last_ts)
        bitrate = (double) i_file * 8 /
                  ((double) last_ts * 1000 * param->i_timebase_num / param->i_timebase_den);
    else
...
#   define  MAX_PTS_WARNING 3 /* arbitrary */
    ...
    ticks_per_frame = (int64_t) param->i_timebase_den * param->i_fps_den / param->i_timebase_num /
                      param->i_fps_num;
    FAIL_IF_ERROR2(ticks_per_frame < 1 && !param->b_vfr_input, "ticks_per_frame invalid: %"
            PRId64
            "\n", ticks_per_frame);
    ...
    if (!param->b_repeat_headers) {
        ...
        FAIL_IF_ERROR2(x264_encoder_headers(h, &headers, &i_nal) < 0,
                       "x264_encoder_headers failed\n");
        FAIL_IF_ERROR2((i_file = cli_output.write_headers(opt->hout, headers)) < 0,
    ...
    for (; !b_ctrl_c && (i_frame < param->i_frame_total || !param->i_frame_total); i_frame++) {
        ...
        if (pic.i_pts <= largest_pts) {
            if (cli_log_level >= X264_LOG_DEBUG || pts_warning_cnt < MAX_PTS_WARNING)
                x264_cli_log("x264", X264_LOG_WARNING,
                             "non-strictly-monotonic pts at frame %d (%"PRId64" <= %"PRId64")\n",
        ...
        if (opt->b_progress && i_frame_output)
            i_previous = print_status(i_start, i_previous, i_frame_output, param->i_frame_total,
    ...
    if (i_frame_output == 1)
        ...
        duration = (double) (2 * last_dts - prev_dts - first_dts) * param->i_timebase_num /
                   param->i_timebase_den;
    else
        duration = (double) (2 * largest_pts - second_largest_pts) * param->i_timebase_num /
                   param->i_timebase_den;
