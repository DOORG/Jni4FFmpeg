    ...
    if (ost->enc_ctx->pix_fmt != AV_PIX_FMT_NONE) {
        ...
        if (ost->enc_ctx->strict_std_compliance <= FF_COMPLIANCE_UNOFFICIAL) {
            if (ost->enc_ctx->codec_id == AV_CODEC_ID_MJPEG) {
                p = (const enum AVPixelFormat[]) {AV_PIX_FMT_YUVJ420P, AV_PIX_FMT_YUVJ422P,
                                                  AV_PIX_FMT_YUV420P, AV_PIX_FMT_YUV422P,
                                                  AV_PIX_FMT_NONE};
            } else if (ost->enc_ctx->codec_id == AV_CODEC_ID_LJPEG) {
                p = (const enum AVPixelFormat[]) {AV_PIX_FMT_YUVJ420P, AV_PIX_FMT_YUVJ422P,
                                                  AV_PIX_FMT_YUVJ444P, AV_PIX_FMT_YUV420P,
...
static int configure_input_audio_filter(FilterGraph *fg, InputFilter *ifilter,
    ...
    if (ist->dec_ctx->channel_layout)
        av_bprintf(&args, ":channel_layout=0x%"
    PRIx64,
