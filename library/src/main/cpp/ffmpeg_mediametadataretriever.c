...
const char *extract_metadata_from_chapter(State **ps, const char *key, int chapter) {
    ...
    return extract_metadata_from_chapter_internal(state->pFormatCtx, state->audio_st,
                                                  state->video_st, key, chapter);
}
...
void convert_image(State *state, AVCodecContext *pCodecCtx, AVFrame *pFrame, AVPacket *avpkt,
    ...
    *got_packet_ptr = 0;
    ...
    if (ret >= 0 && state->native_window) {
        ANativeWindow_setBuffersGeometry(state->native_window, width, height,
